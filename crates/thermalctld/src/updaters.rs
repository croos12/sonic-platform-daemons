...
impl TemperatureUpdater {
    ...
    async fn update_thermal(&self, thermal: &dyn Thermal, _index: usize) -> Result<()> {
        ...
        let low_threshold_val = thermal.get_low_threshold().ok();

        let warning_status = {
            let temp_valid = (temperature - (-999.0)).abs() > 0.1;
            let high_valid = (high_threshold - (-999.0)).abs() > 0.1;

            if temp_valid && high_valid && temperature > high_threshold {
                "True"
            } else if temp_valid && low_threshold_val.is_some() && temperature < low_threshold_val.unwrap() {
                "True"
            } else {
                "False"
            }
        ...
        let format_temp = |temp: f32| {
            if (temp - (-999.0)).abs() < 0.1 {
        ...
        let minimum_temp = match thermal.get_minimum_recorded() {
            Ok(temp) => format!("{:.1}", temp),
            Err(_) => NOT_AVAILABLE.to_string(),
        };

        let maximum_temp = match thermal.get_maximum_recorded() {
            Ok(temp) => format!("{:.1}", temp),
            Err(_) => NOT_AVAILABLE.to_string(),
        };

        let low_threshold_str = match low_threshold_val {
            Some(temp) => format!("{:.1}", temp),
            None => NOT_AVAILABLE.to_string(),
        };

        let critical_low_threshold_str = match thermal.get_low_critical_threshold() {
            Ok(temp) => format!("{:.1}", temp),
            Err(_) => NOT_AVAILABLE.to_string(),
        };

        let is_replaceable_str = match thermal.is_replaceable() {
            Ok(true) => "True",
            Ok(false) => "False",
            Err(_) => "False",
        };
        ...
        let fvs = vec![
            ...
            ("minimum_temperature", minimum_temp),
            ("maximum_temperature", maximum_temp),
            ("high_threshold", format_temp(high_threshold)),
            ("low_threshold", low_threshold_str),
            ("warning_status", warning_status.to_string()),
            ("critical_high_threshold", format_temp(thermal.get_high_critical_threshold().unwrap_or(-999.0))),
            ("critical_low_threshold", critical_low_threshold_str),
            ("is_replaceable", is_replaceable_str.to_string()),
            ("timestamp", timestamp),
        ];
